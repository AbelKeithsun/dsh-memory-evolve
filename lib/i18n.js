...
export const SNAPSHOT_DICT = {
  ...
  'snap.turnEndHead': [
    '- 每轮收尾分两步（不要把完整回复和写入工具调用放进同一条消息——带工具调用的消息结束不了 turn，会逼出多余收尾）：① 本条消息只发写入工具调用（memory/dtodo，不写正文）；② 下一条消息输出完整回复（无工具调用，结束 turn）。',
    '- End of every turn in two steps (do NOT put the complete reply and the write tool calls in the same message — a message with tool calls cannot end the turn, which forces an extra closing step): ① this message carries ONLY the write tool calls (memory/dtodo, no prose); ② the next message outputs the complete reply (no tool calls, ends the turn).',
  ],
