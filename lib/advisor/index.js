...
export function installAdvisor(ctx, config, deps = {}) {
  ...
  disposers.push(ctx.on('session/event', (session, event) => {
    // 修复（issue #49，2026-09-14）：DSH 0.1.2-alpha.4+ 的 Session 不再暴露
    // `.events` 数组（改经 `ownEvents()` 访问），此处仍读旧字段会得到
    // undefined，observer → findLastMessageTurnEnd 对 undefined 做 for...of
    // 立刻抛 `TypeError: events is not iterable`，每个可评审回合报一次。
    // 与 review.js（#38 / #42 同源修复）同款三档兜底：新宿主走 ownEvents()，
    // 老宿主回退 `.events`，两者都拿不到时给空数组（宁可本回合无可评审内容，
    // 也不能让监听器抛错）。
